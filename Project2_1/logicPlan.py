...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    for i in range(li):
        for j in range(i+1, li):
            clause = disjoin([~literals[i], ~literals[j]])
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]=None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    pacmanNow = PropSymbolExpr(pacman_str, x, y, time=now)

    return pacmanNow % disjoin(possible_causes)
    #util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    for x, y in all_coords:
        s1 = PropSymbolExpr(wall_str, x, y) >> ~PropSymbolExpr(pacman_str,x,y,time=t)
        pacphysics_sentences.append(s1)

    s2 = []
    for x,y in non_outer_wall_coords:
        s2.append(PropSymbolExpr(pacman_str,x,y, time=t))
    pacphysics_sentences.append(exactlyOne(s2))

    s3 = []
    for action in DIRECTIONS:
        s3.append(PropSymbolExpr(action,time=t))
    pacphysics_sentences.append(exactlyOne(s3))

    if sensorModel is not None:
        s4 = sensorModel(t,non_outer_wall_coords)
        pacphysics_sentences.append(s4)

    if successorAxioms is not None and t > 0:
        s5 = successorAxioms(t,walls_grid, non_outer_wall_coords)
        pacphysics_sentences.append(s5)
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    q1 = PropSymbolExpr(pacman_str,x1,y1,time=1)
    model1 = findModel(conjoin(KB + [q1]))

    q2 = ~PropSymbolExpr(pacman_str,x1,y1,time=1)
    model2 = findModel(conjoin(KB + [q2]))
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        for (x,y) in non_wall_coords:
            pacmanLocations.append(PropSymbolExpr(pacman_str,x, y, time=t))
        KB.append(exactlyOne(pacmanLocations))
        ...
        for action in actions:
            actionExprsT.append(PropSymbolExpr(action, time=t))
        KB.append(exactlyOne(actionExprsT))
...
def foodLogicPlan(problem) -> List:
    ...
    KB.append(PropSymbolExpr(pacman_str, x0,y0,time=0))
    for (x,y) in non_wall_coords:
        food0 = PropSymbolExpr(food_str,x,y,time=0)
        if (x,y) in food:
            KB.append(food0)
        else:
            KB.append(~food0)
    ...
    for i in range(50):
        PacmanLocation = []
        for (x,y) in non_wall_coords:
            PacmanLocation.append(PropSymbolExpr(pacman_str,x,y,time=i))
        KB.append(exactlyOne(PacmanLocation))

        goalCondition = []
        for (x,y) in food:
            goalCondition.append(~PropSymbolExpr(food_str,x,y,time=i))
        if goalCondition:
            goal = conjoin(goalCondition)
            model = findModel(conjoin(KB + [goal]))
            if model is not False:
                return extractActionSequence(model, actions)
        ...
        actionExprs = []
        for action in actions:
            actionExprs.append(PropSymbolExpr(action, time=i))
        KB.append(exactlyOne(actionExprs))

        if i<49:
            for (x,y)  in non_wall_coords:
                axiom = pacmanSuccessorAxiomSingle(x,y,i+1,walls)
                if axiom:
                    KB.append(axiom)

            for (x,y)  in food:
                food_t  = PropSymbolExpr(food_str,x,y,time=i)
                food_t1  = PropSymbolExpr(food_str,x,y,time=i+1)
                pacman_t  = PropSymbolExpr(pacman_str,x,y,time=i)
                foodAxiom = food_t1 % (food_t  & ~pacman_t)
                KB.append(foodAxiom)
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        perceptRules = fourBitPerceptRules(t, percepts)
