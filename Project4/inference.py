...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        #raiseNotDefined()
        currentListOfFactors = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        for eliminateValue in eliminationOrder:
            unjoinedFactor, joinedFactor = joinFactorsByVariable(currentListOfFactors, eliminateValue)
            currentListOfFactors = unjoinedFactor
            if len(joinedFactor.unconditionedVariables()) > 1:
                currentListOfFactors.append(eliminate(joinedFactor, eliminateValue))

        return normalize(joinFactors(currentListOfFactors))
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldPos in self.allPositions:
            ...
            for nextPos, prob in newPosDist.items():
                newBeliefs[nextPos] += currentBeliefs[oldPos] * prob
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        "*** YOUR CODE HERE ***"
        #raiseNotDefined()
        for posIdx in range(self.numParticles):
            self.particles.append(self.legalPositions[posIdx % len(self.legalPositions)])
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for particle in self.particles:
            particleWeights[particle] += self.getObservationProb(observation, pacPosition, particle, jailPos)
        ...
        for _ in range(self.numParticles):
            newParticles.append(particleWeights.sample())
